...
parser = argparse.ArgumentParser(description="Training of Deep-Clustering")
...
parser.add_argument('--ideal_mask', type=str, default='ibm', choices=['ibm'], help='Ideal mask for target')
...
parser.add_argument('--embed_dim', '-K', type=int, default=40, help='Embedding dimension')
parser.add_argument('--hidden_channels', '-H', type=int, default=300, help='hidden_channels')
parser.add_argument('--num_layers', '-B', type=int, default=2, help='# LSTM layers')
...
parser.add_argument('--add_noise', type=float, default=0, help='Gaussian noise variance.')
...
def main(args):
    ...
    if config_scheduler['scheduler'] is None:
        scheduler = None
    else:
    ...
    trainer = AdhocTrainer(model, loader, wrapper_criterion, optimizer, scheduler, args)
    trainer.run()
