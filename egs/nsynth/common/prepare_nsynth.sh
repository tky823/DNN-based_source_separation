...
if [ -e "${nsynth_root}/${train_dataset}" ]; then
    ...
    wget "${train_url}/${train_dataset}" -P "${nsynth_root}"
    tar -zxvf "${nsynth_root}/${train_dataset}" -C "${nsynth_root}"
fi

if [ -e "${nsynth_root}/${valid_dataset}" ]; then
    echo "Already downloaded dataset ${valid_dataset}"
else
    wget "${valid_url}/${valid_dataset}" -P "${nsynth_root}"
    tar -zxvf "${nsynth_root}/${valid_dataset}" -C "${nsynth_root}"
fi

if [ -e "${nsynth_root}/${test_dataset}" ]; then
    echo "Already downloaded dataset ${test_dataset}"
else
    wget "${test_url}/${test_dataset}" -P "${nsynth_root}"
    tar -zxvf "${nsynth_root}/${test_dataset}" -C "${nsynth_root}"
fi